...
- CONSTRAINT: run `export PR_BODY="$(gh pr view <n> --json body -q .body)" && make check`, 100%
  green, before `gh pr ready`. Consequence: without the export, `scripts/check_docs_impact.py`
  reads an empty body, reports affected pages as unreviewed (or the author never sees the list),
  and CI's required "Check affected documentation" goes red on a gate believed clean (#602, #607,
  #617).
