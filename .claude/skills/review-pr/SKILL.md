...
4. Run `uv run scripts/quality_delta.py` and `PR_BODY="$(gh pr view <n> --json body -q .body)" uv run scripts/concept_budget.py`. Hold the PR to its own `## Expected delta` and `## Concept budget`, which `ship-pr` Stage 1 wrote into the draft body before the first source edit and the design gate already passed. Unexplained overrun beyond roughly 2x runs the overrun pass (`references/overrun.md`); the number itself is never the finding. A body carrying no estimate is a contribution that never ran Stage 1: report the measurement and skip the trigger.
...
| 4. Scope | Should this be two PRs? Is anything here not asked for? Read the final diff as if no review history existed: rounds that left compensating helpers, defensive branches or duplicated tests are a consolidation, not another round. An unexplained overrun runs the overrun pass: `references/overrun.md`. | one line, or the cause the overrun pass named |
...
- **A harness note**, only when the same dev-agent *behavior* (not output) has appeared in two PRs. Establish that with `uv run scripts/finding_ledger.py`, not with recall: the ledger row from the first occurrence is what a later session has instead of memory, and before it existed this bar was reachable only by luck. File the note the same way as a DEFER (`finding: <gap>` issue) but name the behavior, not a proposed rule. Not a new mechanical guard: guards caught 0 of 8 v5 findings, and three of those findings were themselves guard proposals.

**Row append, not optional, and the one thing Harvest always does.** Before the verdict is posted, append one row to `docs/delivery/finding-ledger.md` per BLOCK and per DISCUSS, while the finding is still in hand:

```
| 607 | tests/bindings/test_deck_verbs.py:29 | BLOCK | assertion-cannot-fail |
```

The tag is kebab-case and names the *dev-agent behavior*, never the defect: `assertion-cannot-fail`, not `missing test`. Run `uv run scripts/finding_ledger.py` first and reuse a tag from the histogram whenever one fits. A tag invented per row is how the ledger becomes write-only, since a repeat count is the only thing it is for. Commit that one line on the branch you already checked out in Setup, with the message `ledger: <tag> (#<pr>)`, and push nothing else, ever. It is the single exception to the rule that a reviewer does not write to the tree: the row is evidence, and taste is what that rule protects. Known cost of putting it on the branch: a PR closed unmerged takes its rows with it, which is the cheapest mechanism that keeps the ledger in git rather than in a comment a crawler has to reassemble.
...
- Post measured against declared with the verdict, one line, so the author sees the delta against the estimate Stage 1 wrote. The raw script output stays in the return to the orchestrator.
