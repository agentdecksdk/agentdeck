...
- A public callable's docstring states its contract: what it returns, what it raises or which codes and when. A private helper is never documented more fully than the public symbol it serves (SLOP013 catches the size inversion on changed lines; the sufficiency is yours to judge). The test: for each public callable, name one fact a third party would have to look up elsewhere to use it correctly. If you can name one, that is a finding.
