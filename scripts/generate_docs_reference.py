...
def render_generated_version_ts() -> str:
    """The docs site's footer badge version  -  the newest released heading in `CHANGELOG.md`,
    so a release that forgets to regenerate this fails `make check` instead of shipping stale."""
    current = next(v for v, _d, _b in changelog_sections() if v.lower() != "unreleased")
    return (
        "// Generated by scripts/generate_docs_reference.py. Do not edit by hand.\n"
        f'export const CURRENT_VERSION = "{current}"\n'
    )
...
def _generated_pages() -> dict[Path, str]:
    pages = {
        ...
        VERSION_PAGE: render_generated_version_ts(),
    }
