...
def _log_transcript(history: Sequence[Event]) -> list[Message]:
    ...
    Which ``run.started`` inputs count is the part that is not obvious:
      ...
      | a workflow, tool or skill (#490) | dropped | no model played it: sharing a conversation's ``session_id`` puts the run on that conversation's event stream, where an operator surface keyed by the session reads it, and its input is an orchestration argument nobody said |
    """
