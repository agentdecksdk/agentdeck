"""``ControlPort`` in SQLite: one row per ``ref``, durable enough that a second OS
...
_SCHEMA = "CREATE TABLE IF NOT EXISTS signals (ref TEXT PRIMARY KEY, signal TEXT NOT NULL, reason TEXT);"
...
def _migrate_run_id_to_ref(conn: sqlite3.Connection, db_path: str) -> None:
    """Carry a pre-namespace ``signals`` table (keyed by ``run_id``) forward to the ``ref``
    ...
    keying it by identity (``ref = run_id``) would silently hand it to whichever future caller
    ...
    if "run_id" not in columns or "ref" in columns:
        return  # already on the ref schema, or the table does not exist yet
    ...
    if pending:
        raise StoreError(
            ...
            "namespace, so re-keying them by ref could silently address a different run than the "
...
def _connect(db_path: str) -> sqlite3.Connection:
    ...
    try:
        ...
        _migrate_run_id_to_ref(conn, db_path)
...
class SqliteControlPort(ControlPort):
    ...
    async def signal(self, ref: str, sig: Signal, reason: str | None = None) -> None:
        await self._run(partial(self._write, ref, sig.value, reason), "signal")

    async def poll(self, ref: str) -> ControlSignal | None:
        row = await self._run(partial(self._read, ref), "poll")
    ...
    async def consume(self, ref: str, expected: Signal) -> bool:
        return await self._run(partial(self._take, ref, expected.value), "consume")

    def _take(self, ref: str, sig: str) -> bool:
        ...
        deleted = self._conn.execute("DELETE FROM signals WHERE ref = ? AND signal = ?", (ref, sig))
        self._conn.commit()
        return deleted.rowcount > 0

    def _write(self, ref: str, sig: str, reason: str | None) -> None:
        self._conn.execute(
            "INSERT INTO signals (ref, signal, reason) VALUES (?, ?, ?) "
            "ON CONFLICT(ref) DO UPDATE SET signal = excluded.signal, reason = excluded.reason",
            (ref, sig, reason),
        )
        self._conn.commit()

    def _read(self, ref: str) -> tuple[str, str | None] | None:
        row = self._conn.execute("SELECT signal, reason FROM signals WHERE ref = ?", (ref,)).fetchone()
