...
In ``core`` because every ring raises these, core included: a ``RunContext`` asked to be a
pydantic field refuses with a :class:`ConfigError`, and core may import nothing outside itself.
``agentdeck.errors`` re-exports the whole set, so the import path consumers write is unchanged.
"""
