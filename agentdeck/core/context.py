...
REF_PREFIX = "adr:"
"""Marks a ref as namespace-derived (see :func:`encode`). Reserved: no caller-supplied
``run_id`` may start with it, or an unnamespaced ref could collide with one."""
...
def encode(namespace: str | None, run_id: str) -> str:
    ...
    Derived, not minted — same ``(namespace, run_id)`` always yields the same ref, in any
    process, with no round trip and no mapping table to keep in sync.

    ``encode(None, run_id) == run_id`` is the compatibility keystone: every unnamespaced ref is
    byte-identical to today's ``run_id``, so stored ids, the unnamespaced CLI and the frozen v1
    wire need no migration. A namespaced ref is prefixed and percent-quoted (``safe=""``, so a
    ...
    if namespace is None:
        return run_id
    return f"{REF_PREFIX}{quote(namespace, safe='')}:{quote(run_id, safe='')}"
...
class RunContext:
    ...
    :attr:`ref` is not a fifth value: it is computed from ``namespace`` and ``run_id`` on every
    ...
    *identity* ``namespace`` carefully is not either: ``namespace`` says which runs are kept
    ...
    def __post_init__(self) -> None:
        ...
        if self.run_id.startswith(REF_PREFIX):
            raise ValueError(
                f"run_id must not start with {REF_PREFIX!r}: that prefix is reserved for "
                "namespace-derived refs (see encode()), so a caller-supplied run_id starting "
