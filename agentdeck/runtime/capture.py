...
* **Identity** — ``session_id`` / ``author_id`` are built host-side and
  injected into the sandbox env (see :mod:`agentdeck.runtime.workspace`), so a
  skill can neither forge nor supply them.
...
# The one host↔sandbox wire contract: the env var the Workspace serializes a
