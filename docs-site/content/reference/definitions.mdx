...
| `tools` | `Sequence[Any]` | `base.tools` | plain callables, already-built SDK tools, or a `Workflow` to expose as a tool |
...
booking = Agent(base=BookingBase, name="booking", tools=[find_slots])
