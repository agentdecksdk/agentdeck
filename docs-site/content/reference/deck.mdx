...
`deck.serve(binding, *bindings)` validates a set of bindings and serves them, owning their lifecycle. A
...
Embedding uses `deck.asgi(binding, *bindings)` instead of `serve()`.
