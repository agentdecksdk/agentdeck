...
`deck.serve(*bindings)` validates a set of bindings and serves them, owning their lifecycle. A
binding is one external protocol, channel or surface over one transport. The first one is the
terminal:
...
Embedding uses `deck.asgi(*bindings)` instead of `serve()`.
...
deck = Deck.from_project("./.agentdeck")
asyncio.run(deck.serve(Native.http(), port=8000))
```

`deck.asgi(*bindings)` returns the ASGI application instead, for mounting inside a service you
...
`deck.expose(*bindings)` returns the underlying `Exposure` object directly, for lifecycle control
beyond one call (inspecting it before serving, reusing the same exposure for both `.serve()` and
a health check, closing it manually).
