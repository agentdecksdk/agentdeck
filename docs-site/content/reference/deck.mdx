...
`expose(*bindings)` validates a set of bindings and owns their lifecycle. A binding is one
external protocol, channel or surface over one transport. The first one is the terminal:
...
asyncio.run(deck.expose(Terminal.stdio()).serve())   # what `agentdeck chat` runs
```

Embedding uses `deck.expose(...).asgi()` instead of `serve()`.
...
deck = Deck.from_project("./.agentdeck")
asyncio.run(deck.expose(Native.http()).serve(port=8000))
```

`exposure.asgi()` returns the ASGI application instead, for mounting inside a service you already
run. Its lifespan **opens the deck on startup and closes it on shutdown**, so a served deck needs
no separate `async with`, and a deck you opened yourself is left alone.

```python
app = deck.expose(Native.http(path="/api")).asgi()   # uvicorn yourmodule:app
```
...
```python
app = deck.expose(Native.http(path="/api"), Terminal.stdio()).asgi()
```
