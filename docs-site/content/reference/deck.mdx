...
whatever the code the run reaches needs. A tool that declares an `agentdeck.Context` parameter
receives it as `ctx.data`, by reference; see [Definitions](/reference/definitions) for how a tool
declares one. Three things it deliberately is not: the model never sees it (the context parameter
is absent from the tool schema), it is never written to the event log, and it cannot cross the
HTTP surface — a live Python object has no wire form, so a context-requiring agent is reachable
from an embedded Python caller and not from `asgi()`. Agents only for now: passing `context=` for
a workflow raises rather than accepting a value nothing would read.
...
There is no `context=` on `Deck(...)` itself, and `run(context=)` reaches agent tools only —
workflow nodes, dynamic instructions, hooks and skills do not receive one yet, and `answer()`,
`resume()` and `tick()` do not take one. A constructor-level declaration buys build-time
compatibility checks that do not exist, so it stays absent rather than accepted-and-unused. And
[Run Control](/concepts/run-control#safe-points) reaches agent runs
