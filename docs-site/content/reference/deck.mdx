...
sent: it renders as its own part, `json.dumps(data)` with nothing wrapped around it, since it is
already a separate entry in the model's content list rather than text concatenated into a
caller's own prompt.
...
**The timer sweep (`sleep_until`'s wake-up) is internal only in this release.** Nothing public
lists or resumes a due timer thread yet — a deck-owned clock that will is separate,
not-yet-shipped work. It reads each workflow's own checkpointer rather than the Runtime's log,
...
that default pairing. Its resume, when it runs, goes through the Runtime whenever a due thread
matches a run `runs.pending()` already knows about — closing that run's log entry and freeing its
session claim, the same as `runs.answer()` does — and carries no context, for the same reason
`runs.answer`/`runs.resume` need one supplied: see [below](#where-a-context-does-not-reach).
