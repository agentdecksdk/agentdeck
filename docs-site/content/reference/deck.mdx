...
`value` is passed to the interrupted node unvalidated. The node owns the answer contract: it
must interpret or validate the type and values it expects. Passing a different type or value is
not an `answer()` error unless the node rejects it; otherwise the node may simply interpret it as
a different answer.
