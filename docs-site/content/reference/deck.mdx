...
Deck(
    ...
    context=Calendar,   # the *type* of the per-run context, if this catalog declares one
)
```

`Deck.from_project(path=".agentdeck")` discovers the same four catalog arguments from today's directory
