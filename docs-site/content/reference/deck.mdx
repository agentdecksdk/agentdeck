...
embedded Python caller and not from `asgi()`.
...
There is no `context=` on `Deck(...)` itself, and skills do not receive one yet. `tick()` does not
take one either, so a durable, context-requiring workflow that sleeps is not resumable by the
timer inbox — an autonomous resume has nobody present to resupply what was never written down. A
constructor-level declaration buys build-time compatibility checks that do not exist, so it stays
absent rather than accepted-and-unused. And
