...
`Langfuse()` takes no arguments: it is configured entirely by `AGENTDECK_LANGFUSE_*`, so there is
one place to set the endpoint, environment, sample rate and service name rather than two. Naming
it with no keys configured raises `ConfigError` at open, rather than tracing nothing quietly.
...
<Callout type="info">
  `Langfuse()` needs the `observability` extra: `pip install "agentdeck[observability]"`. Without
