...
A `str` — the common case, and unchanged — or a **list of content blocks**, which is how a turn
carries anything that is not text. A string is coerced to one `text` block on the way in, so the
two forms meet as the same thing before any engine sees them.
