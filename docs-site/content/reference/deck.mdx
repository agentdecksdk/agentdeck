...
Deck(
    ...
    context=None,       # reserved; see "What this does not cover"
)
```
...
`context=` is accepted and stored for forward compatibility, but a non-`None` value at
`run`/`stream`/`resume` raises: full `Context[T]` injection is its own, larger effort and is not
wired into this slice. And [Run Control](/concepts/run-control#safe-points) reaches agent runs
