...
Deck(
    ...
    sinks=[my_audit_sink],          # taps on the event stream; None reads settings, () means none
)
```
...
- **`async with deck:`** starts everything `build()` left alone: the real engines, the event
  store, the session factory, the event sinks, and every configured MCP server. This is what a
...
The event log is the hub. A **sink** is a read-only tap on it — telemetry, cost accounting,
audit — and a deck can have as many as it likes. `sinks=` is where they are declared, and they
open with the deck.
...
deck = Deck(agents=[booking], sinks=[CostSink()])
...
| `sinks=` | What opens |
|---|---|
| *(omitted, or `None`)* | The configured Langfuse sink if `AGENTDECK_LANGFUSE_PUBLIC_KEY` and `AGENTDECK_LANGFUSE_SECRET_KEY` are both set (see [Settings](/reference/settings)); nothing otherwise, with no warning. |
| `[sink, …]` | Exactly these, in order. Naming any sink suppresses the settings-derived Langfuse one — a deck told which taps to open does not open a second one behind your back. |
| `()` | None at all, even where the environment configures Langfuse. |

The lifecycle is the one the rest of the deck follows:

- **`build()`** checks that everything in `sinks=` is an `EventSinkPort` and does nothing else.
  No sink is opened, no telemetry client is constructed, no exporter contacted — so a deck with
  Langfuse configured still validates in CI with nothing reachable.
- **Opening** builds the settings-derived sink (once) and registers every sink before the
  Runtime exists, so no run can be the thing that turns observability on.
- **Closing** tells every sink its stream has ended, which is what makes it write out whatever
  it buffered. What the deck *constructs* it owns; a sink instance you passed in is still yours
  afterwards, and the deck builds no Langfuse client when you supplied your own sinks.

A sink is fire-and-forget by contract: each one has its own bounded queue, one that is slow or
raises costs its own backlog and never a run, and one that keeps failing is disabled and later
retried. A sink that cannot afford to lose an event reads the store instead.
...
Agents SDK, and nothing opens a span outside a sink. A direct `Workflow.run()`, which bypasses
the Runtime, therefore bypasses the sinks too.

There is no `deck.sinks` property, for the same reason there is no `runtime` or `store`: nothing
needs one, and adding a property later is additive while removing one is not.

<Callout type="info">
  Langfuse needs the `observability` extra: `pip install "agentdeck[observability]"`. Without it,
  a deck with no Langfuse keys is unaffected — the SDK is only imported when a sink is actually
  built for it.
</Callout>
