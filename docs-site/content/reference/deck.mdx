...
or a workflow node that declares an `agentdeck.ToolCtx` parameter receives it as `ctx.data`, by
...
| `await run` | | the result: a `TurnResult` for an agent, its own result for a workflow  -  a native body's return value, a graph's final state |
...
that graph declaring `ToolCtx[T]`. The sweep's resume supplies no context  -  an autonomous resume
...
catalog, and those paths never install it, so a `ToolCtx[...]` node reached that way dies with
...
between stream items or before a tool is dispatched, a graph workflow at a node boundary, a native
workflow at its own `ctx.safepoint()`. One limit is left on the graph path  -  a graph that paused
can only be resumed when it is `durable = True`, since a non-durable one compiles with no
checkpointer to continue from, and the resume raises `ConfigError` saying so.
