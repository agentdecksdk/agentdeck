...
| `OPENAI_TRACING_API_KEY` | `str` or `None` | `None` | Legacy OpenAI-native tracing key. Not read anywhere in the current codebase — host tracing runs through Langfuse/OpenInference (`agentdeck.runtime.observability`) instead; kept as a field for backward compatibility with any config still setting it. |
