---
title: Sessions
description: Keep an agent's message history across multiple runs, and what happens when two runs reach for the same one at once.
---

{/* docs_sources:
  - "agentdeck/adapters/executors/openai_agents/sessions.py"
...
## Only an agent turn is a turn

A session carries two things, and they are separate: the **event stream** every run on it
appears in, and the **message history** the model is handed. A workflow, tool or skill run
joins the first without joining the second. Its input is an orchestration argument, not
something a person said, so it never reaches a later model call as a user message.
...
it cost a duplicated first message.
