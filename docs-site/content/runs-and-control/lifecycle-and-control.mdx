...
description: The six states a run can be in, what moves it between them, and which ones you can act on.
---
...
`resume()` on a `running` run and `answer()` on anything but `waiting_answer` are refused, not
merely no-ops: see `PRECONDITIONS` in `agentdeck/core/status.py` for the exact verdict and reason
