...
`paused` and `waiting_answer` are the two resumable states. The three terminal states accept
nothing: a completed run cannot be paused, and a cancelled run cannot be resumed.
