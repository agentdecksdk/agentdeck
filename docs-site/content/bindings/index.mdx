...
Serve every binding a Deck exposes from one call:
...
`deck.serve(...)` validates the set of bindings and owns their lifecycle: one process, one port,
three protocols reaching the same runs.
