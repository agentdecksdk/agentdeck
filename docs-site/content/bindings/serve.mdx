...
`deck.serve(binding, *bindings, host=, port=)` validates the bindings, opens the Deck, starts
every binding, and blocks until interrupted. At least one binding is required: `binding` is
positional, so `deck.serve()` alone is a `TypeError` before any binding code runs.
...
asyncio.run(deck.serve(Native.http("/api"), host="0.0.0.0", port=8000))
```
...
`deck.expose(binding, *bindings)` is the lower-level call underneath both: it returns the
