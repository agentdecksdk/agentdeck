...
| A graph `interrupt()` names no options, so `answer()` takes whatever it is given and the node owns interpreting it. A native `ctx.ask(options=...)` is validated instead | [#235](https://github.com/agentdecksdk/agentdeck/issues/235) |
