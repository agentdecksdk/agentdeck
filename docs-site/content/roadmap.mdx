...
durable operation/replay model — while LangGraph and anything else become invocables wrapped
behind one boundary, auto-wrapped where `.agentdeck/` finds them.
...
and resume with the answer without handing the conversation over — which lands here rather than
