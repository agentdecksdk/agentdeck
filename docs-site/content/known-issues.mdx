...
SQLite. `deck.runs.pending()` reads the event log; the timer path reads the checkpointer. After a
restart the two disagree, and a parked approval can never be answered because `runs.pending()`
...
A worker killed while its run was **running** leaves its `session_id` unusable for
`AGENTDECK_RUNTIME_STALE_RUN_AFTER_SECONDS`, which defaults to **3600**. There is no way to
release it sooner. This does not apply to a worker killed while its run was paused or waiting
on an answer — that session holds until the run is resumed, answered, or cancelled, on purpose,
...
Everything still open above is on **v3.2 — hardening**, which exists for exactly this list. See
the [Roadmap](/roadmap).
