...
Everything up to `agentdeck-serve` or `uvicorn app:server` is covered elsewhere on this site.
...
`agentdeck-serve` is `Deck.from_project().asgi()` behind a console script: it reads `HOST`/`PORT`
from the environment (default `0.0.0.0:8000`) and must run from the directory holding
`.agentdeck/`, since there is no `--project-dir` flag. An application composing its own `Deck`,
like Jack, runs the same way under a plain `uvicorn module:app`.
...
`agentdeck-serve`'s built-in `/health` is `{"status": "ok", "agents": [...], "workflows": [...],
"skills": [...]}` once the Deck has opened, `503 {"status": "starting"}` before its lifespan has
run. That proves the catalog built; it does not touch the event store. A Postgres event store
gone unreachable *after* startup still answers `200`. Jack's own `/health`
...
`agentdeck-serve`'s streamed routes already send `Cache-Control: no-cache` and
`X-Accel-Buffering: no`, the header nginx needs to stop buffering a proxied response. Without it,
nginx holds the whole SSE stream until the run ends, which looks exactly like a hung agent from
the client's side. A hand-rolled route, like Jack's own (which sends only `Cache-Control:
no-cache`), does not get this for free: set `proxy_buffering off;` for that location in nginx
instead, or add the header yourself.
