...
Everything up to `uvicorn app:server` (or `deck.expose(...).serve()`) is covered elsewhere on
this site. This page is what comes after: a worked systemd deployment that starts on boot,
restarts on failure, and shuts down without losing a run in flight.
...
A `Deck` served over HTTP, `app = deck.expose(...).asgi()` or an application composing its own
ASGI app around one (like Jack, below), runs the same way: under a plain `uvicorn module:app`.
...
There is no packaged `/health` route: a binding's own routes are its protocol's, not an
operational one. Add your own the way Jack does (`{"status": "ok", "pages": N}`), and know its
gap going in: it counts loaded documentation pages, not store connectivity. A store gone
unreachable after startup still answers `200` from a route that only checks the catalog built.
...
every observer and the store. `expose(...).asgi()` and Jack's own `async with Deck(...)` both
call it from the ASGI lifespan's shutdown phase, which is what `SIGTERM` triggers by default
(systemd's default `KillSignal`, and uvicorn's own).
...
never runs, in-flight runs never get their `run.cancelled`, and buffered reports never flush.
...
`Native.http()`'s streamed routes already send `Cache-Control: no-cache`, but not
`X-Accel-Buffering: no`, the header nginx needs to stop buffering a proxied response entirely.
Without either, nginx holds the whole SSE stream until the run ends, which looks exactly like a
hung agent from the client's side. Set `proxy_buffering off;` for that location in nginx instead,
or add the header yourself; a hand-rolled route, like Jack's own, needs the same treatment.
...
- [Deck](/reference/deck) - `expose()`, `aclose()`, and every other `Deck` method
