...
log. Advisory: reporting never changes run status, and outside a wired run (no buffer attached)
each method still validates and drops silently rather than raising inside your tool.
