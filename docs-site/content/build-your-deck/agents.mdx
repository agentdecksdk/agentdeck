...
Prefix `model=` to select a provider. Each provider reads its own credential at the actual model
call; a missing or invalid one surfaces there, not from `Deck.build()`.
