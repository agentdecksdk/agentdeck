...
`answer()` passes its value to the interrupted node without validating it. That has to be the
node's job because only the node knows its answer contract: `_confirm` above expects exactly the
strings `"yes"` and `"no"`, while another node could expect a number or a structured value. If
the node did not check, an unexpected value such as `12345` would not make `answer()` fail — the
comparison would simply treat it as a declined quote.
