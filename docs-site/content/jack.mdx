...
<Contribute
  ...
  difficulty="small"
  scope="diagram"
  need="The path from a question to the rendered tree is drawn as a plain-text block, which does not show that the event object crosses every boundary unchanged."
  because="A diagram makes the no-translation-layer claim visible in one glance."
/>
