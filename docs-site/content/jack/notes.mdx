...
## Its own route, not `expose(...).asgi()`

AgentDeck packages an HTTP binding and Jack writes his own route instead: a run started through
`expose(...).asgi()` carries `context=None`. There is no wire form for a live Python object, and
both tools need the `DocsCorpus`.
