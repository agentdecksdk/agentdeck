...
`memory` by default, and refuses to open at all if `AGENTDECK_CONTROL_BACKEND` names anything
else, which surfaces at startup rather than on a `pause`/`cancel` request. That 503 is reachable
only from an embedder who builds a `Runtime` directly with no `ControlPort`, bypassing `Deck`
and `agentdeck-serve` entirely — not a state a deployment following this page can reach.
