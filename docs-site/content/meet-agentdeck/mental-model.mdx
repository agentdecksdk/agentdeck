...
None of them execute anything on their own. A declaration is a description, and it stays inert
until a `Deck` compiles it.
...
a durable id, a status, an owner session, and a log. It outlives the handle you got it from:
...
| a session | the conversation it belongs to, or its own id if it belongs to none |
