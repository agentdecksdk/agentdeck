...
`deck.session_for(session_id)` is a thin lookup, not a store of its own: `AGENTDECK_SESSION_REDIS_URL`
...
held in the `Deck` instance that created it — gone the moment that process exits, invisible to
