...
workers can share — SQLite needs shared memory between processes, so one file behind more than one
machine is not supported. The Python API (`Deck.run`, `Deck.stream`) plays the turn on the same
