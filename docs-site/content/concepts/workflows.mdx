...
async def main() -> None:
    async with Deck.from_project() as deck:
        ...
        if paused["type"] == "interrupt":
            [mine] = [p for p in await deck.runs.pending() if p.thread_id == "quote-42"]
            await deck.runs.answer(mine.run_id, "yes")
...
AgentDeck runs no daemon, and today there is no public way to drive this sweep either: an
internal listing finds timer threads whose moment has passed, and an internal resume plays each
one — but nothing calls either yet, so a due `sleep_until` only wakes once something else (a
future deck-owned clock) drives it.
...
`deck.runs.pending()` already knows about, so that resume itself is recorded in the log like any
