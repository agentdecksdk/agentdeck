...
- **Unnamespaced.** The HTTP surface runs every request without a namespace, so one log space
  serves the whole deployment — `RunContext` carries `namespace`, but nothing
