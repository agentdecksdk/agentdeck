...
```python no-test reason="run_agent, chat and chat_stream are already runnable in concepts/agents"
async for event in app.chat_stream("Support", "wa-123", "and now?"):
...
control](/operating/pause-resume-cancel). Every one of those handlers builds a `RunContext` and
plays it on the same `Runtime` the Python API uses — one canonical log either way.
...
- **One tenant, one principal.** The HTTP surface stamps every request with the same hardcoded
  `tenant="local"`, `principal="user:local"` — `RunContext` carries both fields, but nothing
...
- **A second, native HTTP surface exists but is not reachable.** `agentdeck/surfaces/serve/app.py`
  ...
  enough, not something `pip install agentdeck` gives you to run).
