...
can share. SQLite's cross-process story is a shared *file*, not shared memory: several
processes on the same machine can open it, but a file can't reach a second machine — which is
exactly why `redis` and `postgres` exist as separate options rather than "just point everyone at
the same file."
