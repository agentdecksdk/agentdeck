...
paused, holding its session **forever** — no `stale_run_after` timer reaches a paused or waiting
run, deliberately (see [Sessions and
Memory](/concepts/sessions-and-memory#one-turn-at-a-time)) — until a `resume` lifts it or a
`cancel` ends it.
