...
- **`AGUI.http()`, the AG-UI protocol binding** (#595/#596, slices AGUI-0 through AGUI-3):
  ...
  `deck.expose(AGUI.http("/agui"))` for the whole catalog, or `AGUI.http("/support",
  target="Support")` pinned. Official `ag-ui-protocol` models and `EventEncoder` over HTTP/SSE:
