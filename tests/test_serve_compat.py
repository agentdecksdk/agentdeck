...
def run_context(session_id: str | None = None) -> RunContext:
    """A reader context for the store assertions below.

    The Runtime takes options now, not a context — but ``EventStorePort`` still takes one,
    because it is an internal port. Only the session id and namespace are read off it here.
    """
...
async def test_chat_frames_render_deltas_then_done_from_canonical_events(project, scripted):
    ...
    frames = [frame async for frame in chat_frames(runtime.run("Greeter", coerce_input("when?"), session_id="s1"))]
...
async def test_a_disconnect_closes_its_run_in_the_log(project, scripted):
    ...
    async def consume() -> None:
        async for frame in chat_frames(runtime.run("Greeter", coerce_input("hi"), session_id="s1")):
            frames.append(frame)
...
async def test_a_structured_output_survives_the_canonical_stream(project, scripted):
    ...
    body = await chat_result(runtime.run("Structured", coerce_input("hi"), session_id="s1"))
