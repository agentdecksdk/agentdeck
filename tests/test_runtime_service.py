...
async def test_run_started_carries_what_the_run_was_asked_for() -> None:
    """No context snapshot any more: everything it held was recorded and read by nothing, so
    ``run.started`` carries the ask itself and the envelope carries where it ran."""
    runtime, _ = _runtime()
    opening = [
        event
        async for event in runtime.run(
...
async def test_drain_waits_for_the_sink_emits_still_in_flight() -> None:
    ...
    recorder = Recorder()
    runtime, _ = _runtime(sinks=[recorder])
    async for _ in runtime.run("Greeter", INPUT, run_id=CTX.run_id, session_id=CTX.session_id, namespace=CTX.namespace):
        pass
    await runtime.drain()
...
async def test_a_resumed_run_can_report_too() -> None:
    ...
    async for _ in runtime.run("Asker", INPUT, run_id=CTX.run_id, session_id=CTX.session_id, namespace=CTX.namespace):
        pass
    resumed = [
        event
        async for event in runtime.resume(
