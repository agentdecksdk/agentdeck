...
The [GitHub Project](https://github.com/users/sagi5060/projects/5), filtered to `label:finding`,
is authoritative for what is currently open. The sections below are the investigation and the
disposition, which do not change on their own. Regenerate this table with `make roadmap-sync`.
