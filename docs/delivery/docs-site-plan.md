...
**Status:** Design of record for the documentation site. `.claude/skills/release/SKILL.md` cites DS-D3, `docs-site/README.md` cites DS-D8, `tests/test_docs_examples.py` cites its §6.

**Reference:** the product requirements record (personas §2, FRs §4, phasing §6, metrics §7) ·
...
| DS-D3 | No version switcher. A version badge in the docs shell states the release the site documents. Snapshot a version only when a release removes or changes a documented surface, not per release | v3.0.0, v6.0.0 and #604 each removed a documented surface, so a reader on an older install can read pages describing behavior their version does not have. The badge converts that into an informed read; v6.0.x patches document the same surface and owe no snapshot, the next removal does. |
...
| DS-D10 | The site owns the *user-facing* contract for run control (safe points, restart semantics); `docs/` owns the internal mechanism, and epic Story 3's AC points at the site page | Otherwise safe-point semantics get two homes and we recreate the precedence ambiguity the project index exists to kill. |
