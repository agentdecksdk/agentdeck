...
| Slop firewall v0.2: PreToolUse write gate (slop denied before disk), SLOP004 concrete-placeholder, SLOP005 swallowed-exception (broad catches), SLOP006 blanket-suppression, SLOP008 untracked-skip, coded escape hatch `slopcheck: allow SLOPxxx reason` | `scripts/slopcheck.py`, `.claude/settings.json` | Prevention beats correction: the write gate denies the tool call itself and holds under bypass mode. Every rule sweep-verified zero-FP; NotImplementedError hooks and narrow typed excepts exempted because they are this repo's own idiom |
| `docs/patterns/`: errors, comments, lifecycle, tests as real good/bad pairs | `docs/patterns/`, linked from `CLAUDE.md` | Taste encoded as exemplars is the only form that survives multi-turn generation; files grow via the reviewer's promotion loop |
| deck-dev restructured into staged workflow: Understand (spec gate) -> Design (reuse analysis, analog, concept budget, expected delta in PR body before any source edit) -> Implement -> 10-question diff self-review -> Gate; evaluation contract stated up front | `.claude/agents/deck-dev.md` | Aider's architect/editor result: separating design from editing measurably improves output; the agent knowing its review contract before writing shapes generation, not just selection |
| `scripts/concept_budget.py` + CI: new classes/public symbols/modules/dependencies must not exceed the PR's own `## Concept budget` declaration; public symbols also require the reuse analysis | `scripts/`, `.github/workflows/ci.yml`, PR template | Concepts are what agents generate too cheaply; undeclared abstraction becomes mechanically illegal instead of subjectively "overengineered" |
| deck-reviewer: sibling comparison (verify the code matches its declared analog and 2-3 canonical siblings), rubric verdict, ERROR/WARNING/NOTE finding classes | `.claude/agents/deck-reviewer.md` | "Looks like neighboring canonical code" made a checkable claim; severity classes keep NOTEs from blocking |
| `asyncio.create_task` banned outside the runtime (ruff TID251, owners enumerated) | `pyproject.toml`, `docs/patterns/lifecycle.md` | The ast-grep candidate invariants reduced to native tools: adapter isolation was already import-linter law, task ownership is one banned-api line; ast-grep stays shelved with zero rules lost |
