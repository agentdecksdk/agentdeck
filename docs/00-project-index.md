...
| 19 | `design/runtime/` (the contract set behind its [README](design/runtime/README.md), plus [gaps.md](design/runtime/gaps.md)) | **The Runtime 5.1 target contract**: the eight-state lifecycle, control precedence and safe points, durable ask identity, the injection inbox, the execution tree, the event model, projections, observation, recovery and the public surface. A target, not the runtime as built: docs #12, #15, #16 and #17 stay the as-built record until each area migrates (§2 rule 8) | engineers |
