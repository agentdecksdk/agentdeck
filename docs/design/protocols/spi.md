...
A plugin declares `spi_version = 1` in its `ProtocolInfo`; `expose()` refuses a binding whose `spi_version` this AgentDeck does not support, naming both versions, before anything opens. Breaking `ProtocolGateway`, `ProtocolBinding`, endpoint contracts or `GatewayFailureCode` bumps the major; adding optional fields or capabilities does not.
