...
A plugin declares `spi_version = 1` in its `ProtocolInfo`. Breaking `ProtocolGateway`, `ProtocolBinding`, endpoint contracts or `GatewayFailureCode` bumps the major; adding optional fields or capabilities does not.
