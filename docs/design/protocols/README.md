...
| protocols live in one place | `agentdeck-v2-architecture.md` says `adapters/protocols/` (Ring 2); `engineering/architecture.md` gives `surfaces/` "protocol ingress" | ruled: SPI in `agentdeck/protocols/`, bindings in `adapters/protocols/<name>/`, `surfaces/` deleted |
