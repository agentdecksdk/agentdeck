...
| protocols live in one place | the v2 architecture record says `adapters/bindings/` (Ring 2); `engineering/architecture.md` gives `surfaces/` "protocol ingress" | ruled: SPI in `agentdeck/bindings/`, bindings in `adapters/bindings/<name>/`, `surfaces/` deleted |
